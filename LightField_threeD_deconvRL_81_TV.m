%% （选做）光场三维解卷积实验：添加 TV 正则项
clc; clear;

% ========== 1. 配置路径和参数 ==========
% 读取光场重排后的图像文件
TargetFile = './LightField_Data/results/reduced_images/Realign_Image_9x9.tif'; % 降采样后的 TIFF 文件
View_Num = 9; % 降采样后用于计算的微透镜阵列的尺寸 (中心9×9)
psfPath = './Data/PSF/Reduced_3Dpsf_9x9.mat'; % 降采样的PSF
psfData = load(psfPath); % 加载 PSF 文件
psf = single(psfData.reduced_psf); % PSF 文件，5D维度：[rows, cols, View_Num, View_Num, znum]
[max_row, max_col, u_num, v_num, znum] = size(psf); % 提取 PSF 维度

% RL 解卷积参数
maxiter = 3; % 最大迭代次数
lambda_tv = 0.05; % TV 正则化强度
...
% 定义正向和反向投影函数
forwardFUN = @(Xguess, psf_uv) forwardProjectACC(psf_uv, Xguess);
backwardFUN = @(projection, psf_uv) backwardProjectACC(psf_uv, projection);
...
for iter = 1:maxiter
    ...
    for u = 1:View_Num
        for v = 1:View_Num
            ...
            % 计算误差
            errorEM = single(Current_View_3D) ./ (HXguess + 1e-8); % 防止除零
            ...
            % 消除噪声并更新估计
            Htf(Htf < 1e-4) = 0; % 去除小值，防止过小的更新因子
            Xguess_add = Xguess .* XguessCor ./ Htf; % 更新估计值    

            % 添加 Total Variation 正则化项
            tv_grad = computeTVGradient(Xguess); % 计算 TV 梯度
            Xguess_add = Xguess_add - lambda_tv * tv_grad; % 添加 TV 正则项
...
resultFile = fullfile(reconPath, '3D_Reconstructed_Image_81_1_TV.tif');
imwriteTFSK(uint8(real(Xguess)), resultFile);
disp(['3D 重建结果已保存至: ', resultFile]);

% ========== 6. 函数实现 ==========
% 正向投影
function projection = forwardProjectACC(psf_uv, Xguess) % psf_uv是固定uv值的3D矩阵，第3维为深度
    % 正向投影使用对应视角的 PSF
...
% 反向投影
function Backprojection = backwardProjectACC(psf_uv, projection) % psf_uv是固定uv值的3D矩阵，第3维为深度
    % 翻转 PSF
...
% 计算 Total Variation 梯度
function tv_grad = computeTVGradient(Xguess)
    % 使用梯度法计算 Total Variation Loss 的梯度
    [dx, dy, dz] = gradient(Xguess);
    tv_grad = div(dx ./ (sqrt(dx.^2 + 1e-8)), ...
                  dy ./ (sqrt(dy.^2 + 1e-8)), ...
                  dz ./ (sqrt(dz.^2 + 1e-8))); % 防止除零
end

% 梯度的散度 (divergence)
function div = div(dx, dy, dz)
    [dxx, ~, ~] = gradient(dx);
    [~, dyy, ~] = gradient(dy);
    [~, ~, dzz] = gradient(dz);
    div = dxx + dyy + dzz;
